...
class AuthClient {
  ...
  async checkGoogleUser(email: string): Promise<boolean> {
    try {
      ...
      console.error(
        "[AUTH CLIENT] Google user check error:",
        error.response?.data || error.message
      );
      return false;
    }
  ...
  async loginWithGoogle(email: string): Promise<User> {
    try {
      ...
      console.error(
        "[AUTH CLIENT] Google login error:",
        error.response?.data || error.message
      );
  ...
  async sendOtp(
    ...
    try {
      ...
      const endpoint =
        type === "reset" ? "/auth/forgot-password" : "/auth/resend-otp";
      const payload =
        type === "reset" ? { email } : { email, type: "EMAIL_VERIFICATION" };
      const response = await axios.post(endpoint, payload);
      console.log(`[AUTH CLIENT] ${type} OTP response:`, response.data);
    } catch (error: any) {
      console.error(
        `[AUTH CLIENT] Send ${type} OTP error:`,
        error.response?.data || error.message
      );
      throw new Error(
        error.response?.data?.error || `Failed to send ${type} OTP`
      );
    }
  }
  ...
  async verifyOtp(email: string, otp: string): Promise<void> {
    try {
      ...
      console.error(
        "[AUTH CLIENT] OTP verification error:",
        error.response?.data || error.message
      );
  ...
  async resetPassword(
    ...
    try {
      ...
      console.error(
        "[AUTH CLIENT] Password reset error:",
        error.response?.data || error.message
      );
  ...
  async refreshToken(): Promise<void> {
    try {
      ...
      console.error(
        "[AUTH CLIENT] Token refresh error:",
        error.response?.data || error.message
      );
