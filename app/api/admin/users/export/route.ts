...
export async function GET(request: NextRequest) {
  try {
    ...
    const chats = await Chat.find({ $or: [ { user1: { $in: userIds } }, { user2: { $in: userIds } } ] }).lean();
    ...
    const reviewsGiven = await Review.find({ reviewer_id: { $in: userIds } }).lean();
    const reviewsReceived = await Review.find({ reviewed_user_id: { $in: userIds } }).lean();
    ...
    const reportsFiled = await Report.find({ reported_by: { $in: userIds } }).lean();
    ...
    reportsReceived.forEach(r => {
      ...
      const product = products.find(p => p._id.toString() === r.product_id.toString());
      if (product) {
    ...
    const manualPayments = await ManualPayment.find({ user: { $in: userIds } }).lean();
    ...
    const withdrawalLogs = await WithdrawalLog.find({ admin: { $in: userIds } }).lean();
    ...
    const announcements = await Announcement.find({ 'targetAudience.userIds': { $in: userIds } }).lean();
    ...
    if (format === 'csv') {
      ...
      const rows = users.map(user => {
        ...
        return {
          ...
          reviewsReceived: JSON.stringify(reviewsReceivedByUser[userIdStr] || []),
          reportsFiled: JSON.stringify(reportsFiledByUser[userIdStr] || []),
          reportsReceived: JSON.stringify(reportsReceivedByUser[userIdStr] || []),
