  ...
  try {
    ...
    const results = await Promise.all(users.map(async (user: any) => {
      ...
      const stats = {
        ...
        expiredListings: listedProducts.filter((p: any) => p.status === "expired").length,
        removedListings: listedProducts.filter((p: any) => p.status === "removed").length,
        pendingListings: listedProducts.filter((p: any) => p.status === "pending").length,
        featuredListings: listedProducts.filter((p: any) => p.featured === true).length,
