...
export type UserSubscriptionStatus = "pending" | "active" | "expired" | "cancelled" | "suspended";
...
export interface IUserSubscription extends Document {
  ...
  cancel(cancelledBy: mongoose.Types.ObjectId, reason?: string): Promise<IUserSubscription>;
}

export interface IUserSubscriptionModel extends Model<IUserSubscription> {
  findActiveByUser(userId: mongoose.Types.ObjectId): Promise<IUserSubscription | null>;
  findPendingByUser(userId: mongoose.Types.ObjectId): Promise<IUserSubscription[]>;
...
userSubscriptionSchema.methods.canPostAd = function(): boolean {
  ...
  const maxAds = this.planType === "basic" ? 20 : 
                  this.planType === "pro" ? 60 : 
...
userSubscriptionSchema.methods.canUseFeaturedAd = function(): boolean {
  ...
  const maxFeaturedAds = this.planType === "basic" ? 0 : 
                        this.planType === "pro" ? 5 : 
                        Number.MAX_SAFE_INTEGER; // VIP has unlimited
...
userSubscriptionSchema.methods.incrementAdUsage = function(): Promise<IUserSubscription> {
  this.adsUsed += 1;
  return this.save();
};

userSubscriptionSchema.methods.incrementFeaturedAdUsage = function(): Promise<IUserSubscription> {
  this.featuredAdsUsed += 1;
  return this.save();
};

userSubscriptionSchema.methods.useHomepageBanner = function(): Promise<IUserSubscription> {
...
userSubscriptionSchema.methods.cancel = function(cancelledBy: mongoose.Types.ObjectId, reason?: string): Promise<IUserSubscription> {
...
// Static methods
userSubscriptionSchema.statics.findActiveByUser = function(userId: mongoose.Types.ObjectId) {
  const now = new Date();
  return this.findOne({
    user: userId,
    status: "active",
...
userSubscriptionSchema.statics.findPendingByUser = function(userId: mongoose.Types.ObjectId) {
  return this.find({
    user: userId,
    status: "pending",
  }).populate("plan");
...
const UserSubscription: IUserSubscriptionModel = mongoose.model<IUserSubscription, IUserSubscriptionModel>("UserSubscription", userSubscriptionSchema);
