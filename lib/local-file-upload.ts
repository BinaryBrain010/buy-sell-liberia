...
async function ensureDirectoryExists(dirPath: string): Promise<void> {
  try {
    ...
    console.error(
      `[LOCAL FILE UPLOAD] Error creating directory ${dirPath}:`,
      error
    );
...
export async function uploadProductImagesToLocal(
  ...
  try {
    console.log(
      `[LOCAL FILE UPLOAD] Uploading ${files.length} images for product ${productId}`
    );
    ...
    // Create directory path: uploads/products/{category}/{uuid_first_5_chars}_{name}/ (at project root)
    ...
    const uploadDir = join(
      ...
      "products",
      sanitizedCategory,
      `${productIdFirst5}_${sanitizedProductName}`
    );
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      // Generate relative path for database storage (remove 'public')
      const relativePath = `/products/${sanitizedCategory}/${productIdFirst5}_${sanitizedProductName}/${filename}`;
      uploadedPaths.push(relativePath);

      console.log(
        `[LOCAL FILE UPLOAD] Image ${i + 1} uploaded: ${relativePath}`
      );
    }
 ...
 */
export async function deleteProductImagesFromLocal(
  imagePaths: string[]
): Promise<void> {
  try {
    ...
    const deletePromises = imagePaths.map(async (imagePath) => {
      ...
      try {
        ...
        console.error(
          `[LOCAL FILE UPLOAD] Error deleting ${imagePath}:`,
          error
        );
 ...
 */
export function validateImageFileForLocal(file: File): {
  valid: boolean;
  error?: string;
} {
 ...
 */
export function validateImageFilesForLocal(files: File[]): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
