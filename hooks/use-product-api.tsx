...
export function useProductsApi() {
  ...
  const buildApiParams = useCallback((params: FetchProductsParams) => {
    ...
    if (params.filters.category)
      urlParams.append("category_id", params.filters.category);
    ...
    if (params.filters.priceMin !== undefined)
      urlParams.append("minPrice", params.filters.priceMin.toString());
    if (params.filters.priceMax !== undefined)
      urlParams.append("maxPrice", params.filters.priceMax.toString());

    // Time filter
    if (params.filters.timeFilter)
      urlParams.append("timeFilter", params.filters.timeFilter);

    // Sort
    if (params.filters.sortBy)
      urlParams.append("sortBy", params.filters.sortBy);
    if (params.filters.sortOrder)
      urlParams.append("sortOrder", params.filters.sortOrder);

    // Condition (array -> comma separated lowercase)
    if (params.filters.condition && params.filters.condition.length > 0) {
      urlParams.append("condition", params.filters.condition.join(","));
    }
